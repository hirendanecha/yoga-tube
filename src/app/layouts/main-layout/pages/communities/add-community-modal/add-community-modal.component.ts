...
export class AddCommunityModalComponent implements OnInit, AfterViewInit {
  @Input() title: string | undefined = 'Apply to be Yoga Instructor';
  ...
  userId = '';
  profileId = '';
  originUrl = environment.webUrl + 'yoga-instructors/';
  logoImg: any = {
    file: null,
    url: '',
  };
        ...
        this.communityService.createCommunity(formData).subscribe({
          next: (res: any) => {
            ...
            if (!res.error) {
              ...
              this.toastService.success(
                'Your Yoga Instructor will be approved within 24 hours!'
              );
          ...
          error: (err) => {
            this.toastService.danger(
              'Please change Instructor name. this Instructor name already in use.'
            );
            this.spinner.hide();
          },
        });
        ...
        .subscribe({
          next: (res: any) => {
            ...
            if (!res.error) {
              ...
              // this.createCommunityAdmin(res.data);
              this.toastService.success(
                'Your changes edit successfully!'
              );
              this.activeModal.close('success');
            }
          },
          error: (err) => {
            this.toastService.danger(
              'Please change Instructor name. this Yoga Instructors name already in use.'
            );
            this.spinner.hide();
          },
        });
